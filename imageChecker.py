from PIL import Image
from fractions import gcd
from crazyFunctions import *
import os, sys, math, io

args = sys.argv

##########################
##Opening the image file##
##########################
myImage = Image.open(str(args[1]))
myImage = myImage.convert("RGB")
xSize, ySize = myImage.size
print("Image Dimensions:\n    Width: " + str(xSize) + " Height: " + str(ySize))

if(args[2] == "m"):
	maxBytes = getMaxBytesGivenPattern(xSize * ySize, 3, matchesPattern)
	print(str(maxBytes) + "/" + str(xSize * ySize) + " bytes available for writing.")
elif(args[2] == "c"):
	#There is a faster way to do this but I just want it to work
	pixList = myImage.getdata()
	col1 = pixList[0]
	col2 = pixList[1]
	col1 = addColor(col1, (9, 9, 9))
	col2 = addColor(col2, (9, 9, 9))
	col1 = subtractColor(col1, pixList[2])
	col2 = subtractColor(col2, pixList[2])
	thousands = colorTupleToBase10(col2)
	ones = colorTupleToBase10(col1)
	byteNum = (thousands * 1000) + ones
	#Reverse the operation and test to make sure it's likely there's a file in here
	col1 = addColor(col1, pixList[2])
	col2 = addColor(col2, pixList[2])
	col1 = subtractColor(col1, base10ToColorTuple(ones))
	col2 = subtractColor(col2, base10ToColorTuple(thousands))
	same = True
	for x, y, z in zip(col1, col2, pixList[2]):
		same = False
		if(x==y and x==z):
			same = True
	if(same and byteNum < 999999):
		#May have false positives, but unlikely
		print("Stored File Size: " + str(byteNum) + " bytes")
	else:
		print("No Stored File.")
else:
	print("Invalid Mode.")